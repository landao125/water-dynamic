...
Program HB_exist_ACF
  ...
  integer,allocatable :: reginAtom(:,:),nhb(:,:),hb(:,:,:)
  ...
  namelist /input/ natom,npatom,nfram,nsite,n_initial,nts,tstep
  ...
  allocate(boxL(nfram))
  allocate(reginAtom(natom,nfram))
  allocate(dis_ij(npatom))
  ...
  allocate(nhb(nOatom,nfram),hb(10,nOatom,nfram))
   ...
   ! any |O-protein| < 3.5A, O atom belong to regin1
   ! all |O-protein| >10.0A and any |O-protein| < 12.0A ,O atom belong to regin2  
    reginAtom=0
    dis_ij=0.0
    loop1:  do ifram=1,nfram
      loop2:  do iatom=npatom+1,natom,nsite
               atomCount=0
        loop3: do jatom=1,npatom
                ...
                elseif(dis_ij(jatom)>10.0)then  
                  ...
                  if(atomCount==npatom .and. ANY(dis_ij<12.0))reginAtom(iatom,ifram)=2
                end if
               end do loop3
  ...
  allocate(hAve1(nts),h0htAve1(nts),ACF1(nts))
  allocate(hAve2(nts),h0htAve2(nts),ACF2(nts))
  ...
  open (unit=101, file='HB_exist_ACF-268K-400ps.out',status='unknown')
  !ACF1(0)=1.0
  !ACF2(0)=1.0
  write(101,'(3(f12.6,2x))') 0.0,1.0,1.0
  do its=1,nts
  ...
  deallocate(nhb,hb)
  deallocate(hAve1,hAve2)
  deallocate(h0htAve1,h0htAve2)
  deallocate(ACF1)
   write(*,*)"14 ok"
  deallocate(ACF2)
   write(*,*)"15 ok"
